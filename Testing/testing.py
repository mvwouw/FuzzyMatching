...
if __name__ == '__main__':
    ...
    # test = geo.get_top('  lAaglandën ', top=0)
    ...
    # test = lang.get_top('  Vërderwegdanallesanderszz ', top=5, look_around=-1, lmin=3)  # 600/515/380 ms
    ...
    if test:
        ...
        for i, result in enumerate(test['results']):
            print(f"{i + 1}. {result[0]}    (from: '{result[1]}', as: '{result[2]}', ratio: '{round(result[4], 2)}', relation: '{result[3]}')")
